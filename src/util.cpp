...
#if (__APPLE__)
      path = fs::path("/private/var/root/.sigil");
#else
      path = fs::path("/root/.sigil");
#endif
    else
      path = fs::path(pszHome) / ".sigil";
#endif
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "sigild.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Sigil will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("Sigil"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
