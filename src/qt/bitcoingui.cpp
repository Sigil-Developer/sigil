...
BitcoinGUI::BitcoinGUI(QWidget *parent):
   ...
   spinnerFrame(0)
{
   setWindowTitle(tr("Sigil") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
   qApp->setWindowIcon(QIcon(":icons/sigil"));
   setWindowIcon(QIcon(":icons/sigil"));
#else
...
#ifndef Q_OS_MAC
       qApp->setStyleSheet("QToolBar QToolButton { text-align: center; width: 100%; \
         color: white; background-color: #3866af; padding-left: 5px; padding-right: 5px; \
         padding-top: 2px; padding-bottom: 2px; margin-top: 2px; } \
         QToolBar QToolButton:hover { font-weight: bold; \
         background-color: QLinearGradient(x1: 0, y1: 0, x2: 1, y2: 2, \
         stop: 0 #1B3354, stop: 1 #7288a8); } \
         #toolbar { border: none; height: 100%; min-width: 150px; max-width: 150px; \
         background-color: #3866af; } \
         QMenuBar { color: white; background-color: #3866af; } \
         ...
         QMenu::item:enabled:selected { color: white; background-color: blue; } \
...
void BitcoinGUI::createActions(int nQtStyle) {
   ...
   sendCoinsAction->setToolTip(tr("Send coins to an Sigil address"));
   ...
   toggleHideAction = new QAction(QIcon(":/icons/sigil"), tr("&Show / Hide"), this);
   ...
   aboutAction = new QAction(QIcon(":/icons/sigil"), tr("&About Sigil"), this);
...
#ifndef Q_OS_MAC
           qApp->setWindowIcon(QIcon(":icons/sigil_testnet"));
           setWindowIcon(QIcon(":icons/sigil_testnet"));
#else
           MacDockIconHandler::instance()->setIcon(QIcon(":icons/sigil_testnet"));
#endif
           if(trayIcon)
           {
               trayIcon->setToolTip(tr("Sigil client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
   ...
   trayIcon->setToolTip(tr("Sigil client"));
...
void BitcoinGUI::setNumConnections(int count)
{
   ...
   labelConnectionsIcon->setToolTip(tr("%n active connection(s) to the Sigil network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
   if(event->mimeData()->hasUrls())
   {
       ...
       if (nValidUrisFound)
           ...
           notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Sigil address or malformed URI parameters."));
   }
