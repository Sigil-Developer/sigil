...
int64 GetProofOfWorkReward(int nHeight, int64 nFees) {
    ...
    if(nHeight > 0)  nSubsidy = 2500000 * COIN;
    if(nHeight > 1) nSubsidy = 25 * COIN;
    if(nHeight > 1900000) nSubsidy >>= ((nHeight - 1) / 1900000);
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Sigil", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew) {
    ...
    if(mapBlockIndex.empty()) {
        ...
        if(!fTestNet) {
            ...
            //  Coinbase(hash=a95b9f457f, nTime=1504267100, ver=2, vin.size=1, vout.size=1, nLockTime=0, strTxComment=text:Sigil genesis block)
            ...
            const char* pszTimestamp = "Sigil Network Initization - 10/Dec/2017";
            txNew.vin.resize(1);
            txNew.vout.resize(1);
            txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(9999) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
            txNew.vout[0].SetEmpty();
            txNew.nTime = 1512948312;
            txNew.strTxComment = "text:Sigil genesis block";
            block.vtx.push_back(txNew);
            block.hashPrevBlock = 0;
            block.hashMerkleRoot = block.BuildMerkleTree();
            block.nVersion = 1;
            block.nTime    = 1512948312;
            block.nBits    = bnProofOfWorkLimit.GetCompact();
            block.nNonce   = 525410;

        } else {

            // The Sigil testnet genesis block:
            ...
            txNew.vout[0].scriptPubKey = CScript() << ParseHex("048332F10BCCDA76F971D6417D420C6BB5735D3286669CE03B49C5FEA07078F0E07B19518EE1C0A4F81BCF56A5497AD7D8200CE470EEA8C6E2CF65F1EE503F0D3E") << OP_CHECKSIG;
            txNew.nTime = 1392724800;
            txNew.strTxComment = "text:Sigil testnet genesis block";
            block.vtx.push_back(txNew);
            block.hashPrevBlock = 0;
            block.hashMerkleRoot = block.BuildMerkleTree();
            block.nVersion = 1;
        ...
        if(!fTestNet) assert(block.hashMerkleRoot == uint256("0xa817d019e778b40ff4c9924f055f0b160201cb73b0f0f6fbfcd4e22bcb419bc6"));
...
unsigned char pchMessageStart[4] = { 0xa3, 0xbc, 0xdf, 0xe2 };
