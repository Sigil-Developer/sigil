...
void ThreadIRCSeed2(void* parg)
{
    ...
    while(!fShutdown) {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #sigilTEST\r");
            Send(hSocket, "WHO #sigilTEST\r");
        } else {
            // randomly join #sigil00-#sigil05
            // int channel_number = GetRandInt(5);

            /* Use a single channel currently */
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #sigil%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #sigil%02d\r", channel_number).c_str());
        }
